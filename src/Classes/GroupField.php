...
class GroupField extends GF_Field {
	...
	public function get_field_input( $form, $value = '', $entry = null, $field = null, $context = null ): string {
		...
		// Note: Gravity Forms core does not expose a get_css_class() method on GF_Field.
		// If needed in the future, use $this->cssClass directly or build classes manually.

		// Group fields are always repeatable
		$repeater_id = "gf-repeater-{$form_id}-{$id}";

		$output = '';

		// Always show repeater controls for Group fields
		$output .= $this->get_repeater_controls( $form_id, $id );

		$output .= sprintf(
			'<div class="ginput_container ginput_container_group">%s</div>',
			$this->get_group_content( $form, $value, $entry, $field, $context )
		);
	 ...
	 * @return string
	 */
	private function get_group_content( $form, $value, $entry, $field, $context ): string {
		...
		if ( $label ) {
			$output .= sprintf( '<h3 class="gf-group-title">%s</h3>', $label );
		}
	...
	private function open_fieldset( $field ): string {
		...
		$classes = [ 'gf-fieldset', 'gf-group-fieldset', 'gf-repeater-fieldset' ];
